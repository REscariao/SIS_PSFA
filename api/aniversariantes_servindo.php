...
try {
    ...
    SELECT 
        ...
        '#81693b' AS corhex, -- Cor padrão para quem está servindo
        tet.Equipe AS nome_circulo
    FROM tabela_equipes_trabalho tet 
    JOIN tabela_membros m ON tet.Cod_Membros = m.codigo 
    WHERE tet.Cod_Encontro = ? AND MONTH(m.nascimento_dele) = ?
    ...
    FROM tabela_equipes_trabalho tet 
    JOIN tabela_membros m ON tet.Cod_Membros = m.codigo 
    WHERE tet.Cod_Encontro = ? AND MONTH(m.nascimento_dela) = ?
) AS lista_niver
