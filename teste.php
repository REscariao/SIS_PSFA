...
// SQL: Busca nomes, encontro, período e a cor do círculo
$sql = "SELECT m.apelido_dele, m.apelido_dela, e.encontro, e.periodo, 
               c.circulo AS cor_nome, c.cor AS cor_hex
...
$paginas = array_chunk($encontristas, 2);
...
<head>
    <meta charset="UTF-8">
    <title>Crachás Coração - Cores Híbridas</title>
    <style>
        ...
        /* Estilo fixo para as informações do encontro */
        .info-evento {
            position: absolute; text-align: center;
            color: #81693b; /* COR FIXA SOLICITADA */
...
<body>
    ...
    <main class="area-impressao">
        <?php foreach ($paginas as $parEncontristas): ?>
            <div class="folha-a4">
                <?php foreach ($parEncontristas as $c): 
                    $imagemFundo = definirImagemCora($c['cor_nome'] ?? '');
                    $corNomes = !empty($c['cor_hex']) ? $c['cor_hex'] : '#1a237e';
                ?>
                    <div class="cracha-container" style="background-image: url('<?php echo $imagemFundo; ?>');">
                        
                        <div class="nomes-esquerda" style="color: <?php echo $corNomes; ?>;">
                        ...
                        <div class="nomes-direita" style="color: <?php echo $corNomes; ?>;">
